...
!> the call 'python /gpfs/home/lemmen/devel/mossco/code/examples/generic/create_coupling.py /gpfs/home/lemmen/devel/mossco/code/examples/esmf/sediment/default--fabm_sediment.yaml'
!>
...
#define ESMF_METHOD "InitializeP1"
  subroutine InitializeP1(gridComp, importState, exportState, parentClock, rc)
    ...
    gridCompNameList(1) = 'default'
    gridCompNameList(2) = 'fabm_sediment'
    ...
    call ESMF_GridCompSetServices(gridCompList(1), default_SetServices, rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) &
      call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
    call ESMF_GridCompSetServices(gridCompList(2), fabm_sediment_SetServices, rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) &
      call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
...
!     TODO: clock provided during Create() seems to be not recognized?!
    ...
    do phase = 1,2
      ...
      if (gridCompPhaseCountList( 1)>= phase .or. gridCompPhaseCountList( 2)>= phase) then
        ...
        write(message,"(A)") trim(myName)//" linking "//trim(gridCompNameList(1))//"Export to "//trim(gridCompNameList(2))//"Import"
        ...
        write(message,"(A)") trim(myName)//" linking "//trim(gridCompNameList(2))//"Import to "//trim(gridCompNameList(1))//"Export"
        call ESMF_LogWrite(trim(message), ESMF_LOGMSG_INFO)
        call ESMF_CplCompInitialize(cplCompList(1), importState=gridImportStateList(2), &
          exportState=gridExportStateList(1), clock=clock, rc=localrc)
        if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) &
          call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
