...
!> the call 'python /sciclone/home20/clemmen/devel/mossco/code/examples/generic/create_coupling.py /sciclone/home20/clemmen/devel/mossco/code/examples/generic/schism-aggregation.yaml'
!>
...
module toplevel_component
  ...
  use time_aggregation_component, only : time_aggregation_SetServices => SetServices 
  use schism_esmf_cap, only : schism_SetServices => SetServices 
...
#define ESMF_METHOD "InitializeP1"
  subroutine InitializeP1(gridComp, importState, exportState, parentClock, rc)
    ...
    gridCompNameList(1) = 'mossco_lowres'
    gridCompNameList(2) = 'mossco_highres'
    gridCompNameList(3) = 'schism'
    gridCompNameList(4) = 'time_aggregation'
    ...
    do i=1, numGridComp
      ...
      if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) &
        call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
    enddo
    !! Adding meta information to output component mossco_lowres
    ...
    do i=1, numGridComp
      ...
      if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) &
        call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
    enddo
    !! Adding meta information to output component mossco_highres
    ...
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) &
      call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
    call ESMF_GridCompSetServices(gridCompList(3), schism_SetServices, rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) &
      call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
    call ESMF_GridCompSetServices(gridCompList(4), time_aggregation_SetServices, rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) &
      call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
...
!     TODO: clock provided during Create() seems to be not recognized?!
    ...
    do phase = 1,2
      ...
      !! Linking
      !! linking mossco_lowres and time_aggregation
      if (gridCompPhaseCountList( 1)>= phase .or. gridCompPhaseCountList( 4)>= phase) then
        !! linking time_aggregationExport to mossco_lowresImport
        ...
        !! linking mossco_lowresImport to time_aggregationExport
      ...
      !! linking mossco_highres and schism
      if (gridCompPhaseCountList( 2)>= phase .or. gridCompPhaseCountList( 3)>= phase) then
        !! linking schismExport to mossco_highresImport
        write(message,"(A)") trim(myName)//" linking "//trim(gridCompNameList(3))//"Export to "//trim(gridCompNameList(2))//"Import"
        call ESMF_LogWrite(trim(message), ESMF_LOGMSG_INFO)
        ...
        !! linking mossco_highresImport to schismExport
        write(message,"(A)") trim(myName)//" linking "//trim(gridCompNameList(2))//"Import to "//trim(gridCompNameList(3))//"Export"
        call ESMF_LogWrite(trim(message), ESMF_LOGMSG_INFO)
      ...
      !! linking schism and time_aggregation
      if (gridCompPhaseCountList( 3)>= phase .or. gridCompPhaseCountList( 4)>= phase) then
        !! linking schismExport to time_aggregationImport
        ...
        !! linking time_aggregationImport to schismExport
...
#if ESMF_VERSION_MAJOR > 7 && ESMF_VERSION_MINOR > 0
   ...
   enddo
    ...
    call ESMF_TimeIntervalSet(alarmInterval, startTime, m=10 ,rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)

    cplAlarmList(1)=ESMF_AlarmCreate(clock=clock,ringTime=startTime+alarmInterval, &
      ringInterval=alarmInterval, name='schism--time_aggregation--cplAlarm', rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
    ...
    do i=1,numGridComp
      ...
      if (trim(childName)=='schism' .or. trim(childName)=='time_aggregation') then
    ...
    cplAlarmList(2)=ESMF_AlarmCreate(clock=clock,ringTime=startTime+alarmInterval, &
      ringInterval=alarmInterval, name='schism--mossco_highres--cplAlarm', rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
    ...
    do i=1,numGridComp
      ...
      if (trim(childName)=='schism' .or. trim(childName)=='mossco_highres') then
        call ESMF_GridCompGet(gridCompList(i), clockIsPresent=clockIsPresent, rc=localrc)
        if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
    ...
    cplAlarmList(3)=ESMF_AlarmCreate(clock=clock,ringTime=startTime+alarmInterval, &
      ringInterval=alarmInterval, name='time_aggregation--mossco_lowres--cplAlarm', rc=localrc)
    if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
    ...
    do i=1,numGridComp
      ...
      if (trim(childName)=='time_aggregation' .or. trim(childName)=='mossco_lowres') then
        call ESMF_GridCompGet(gridCompList(i), clockIsPresent=clockIsPresent, rc=localrc)
        if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, ESMF_CONTEXT, rcToReturn=rc)) call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT)
...
#define ESMF_METHOD "Run"
  subroutine Run(gridComp, importState, exportState, parentClock, rc)
    ...
    !! Running final netcdf output coupling schism to mossco_highres
    ...
    !! Running final netcdf output coupling time_aggregation to mossco_lowres
