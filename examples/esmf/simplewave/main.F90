...
! Initialize
   call ESMF_Initialize(defaultLogFileName="ESMF_stress_example",rc=localrc,&
   ...
   call ESMF_LogWrite("ESMF driver start", ESMF_LOGMSG_INFO)
...
! Create and initialize a clock, with 100 second timestep, and 50000 s running time
   call ESMF_TimeIntervalSet(timeStep, s=100, rc=localrc)
   if (localrc /= ESMF_SUCCESS) call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT) 

   call ESMF_TimeSet(startTime, yy=2004, mm=9, dd=25, s=0, rc=localrc)
   if (localrc /= ESMF_SUCCESS) call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT) 

   call ESMF_TimeSet(stopTime, yy=2004, mm=9, dd=25, s=200,rc=localrc)
   if (localrc /= ESMF_SUCCESS) call ESMF_Finalize(rc=localrc, endflag=ESMF_END_ABORT) 
...
! Destroy toplevel component and clean up
   ...
   call ESMF_LogWrite('ESMF finished on '//timestring,ESMF_LOGMSG_INFO)   
