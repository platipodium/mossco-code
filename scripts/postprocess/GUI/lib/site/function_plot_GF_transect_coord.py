...
modified:nikolai.chernikov.ru@gmail.com
"""
import os
import pickle
import LatLon
...
def main(crd1, crd2, fname, fname_topo, varname, val_org, arlen=100, PRJ_DATA=None, clim=[0, 100]):
    ...
    #make plots
    makeplots(fpath, proj, xx, yx, val_org, xc_tr, yc_tr, val_tr, titlestr, clim)

    return((xc_tr, yc_tr, val_tr))


def makeplots(fpath, proj, xc, yc, val_org, xc_tr, yc_tr, val_tr, titlestr, clim):
    print "making plots..."
    ...
    plot2Dmap(f, ax, clim, xc, yc, val_org, proj)  #background:
...
def plot2Dmap(f, ax, clim, x, y, v0, proj):
    ...
    pcf = proj.pcolormesh(x, y, v0, cmap=plt.get_cmap('rainbow_r'), vmin=clim[0], vmax=clim[1])

    #coastlines, etc
    proj.drawcoastlines(color=(0.3, 0.3, 0.3), linewidth=0.5)
    proj.drawmapboundary(fill_color=(0.9, 0.9, 1.0))
...
def get_projection_extent_from_bathymetry(fname):
    ncB = netCDF4.Dataset(fname)
    ncBv = ncB.variables
    lat_name = ncBv[bathymetry_varname].dimensions[-2]  # str, name of the variable
    lon_name = ncBv[bathymetry_varname].dimensions[-1]  # str, name of the variable
