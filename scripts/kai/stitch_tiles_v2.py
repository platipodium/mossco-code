...
for key,value in nc.variables.iteritems():
  ...
  #try:
  if not (key in set(dimDict.values())):
  ...
  for att in value.ncattrs():
    # ie., changing '_FillValue' cause problems when writing. 'external_index' is potentially another dangerous attr to change
    if (att not in set(('coordinates','_FillValue', 'external_index'))):
      var.setncattr(att,value.getncattr(att))
      #if not (key in set(dimDict.values())):
      #  print att +' : '+str(value.getncattr(att))

  if not (key in set(dimDict.values())):
        print ' old attributes: ' + str(value.ncattrs())
        print ' new attributes: ' + str(var.ncattrs())

  #test writing;
  #if not (key in set(dimDict.values())):
  #  try:
  #    var[:]=value._FillValue
  #  except:
  #    print key
...
for f in files[:]:
  ...
  for key,value in nc.variables.iteritems():
    ...
    if (has3d and value.shape[-2] == y1-y0+1 and value.shape[-1] == x1-x0+1 ) : #
      ...
      print key + ' is neither 2d nor 3d. Skipped'
      break
