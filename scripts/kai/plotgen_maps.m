...
ili=1;
ntagm=ntags+(exist('datat') & strcmp(upper(varshort0),'CHL'));
...
varshortm0=varshort;
if isstrprop(ptag(2), 'digit')
  vli=2:length(ptag); mode='s';
elseif exist('timeg') & strcmp(upper(varshort),'CHL')
...
% zero indicates running index for time slices
 ...
 elseif mode=='c'
  ...
  ti = toffm+((cell2mat(var{i}(6))-moff-1)*(nrowm*ncolm)+im)*7;
  ...
  ti =mdi+toffm;
%%  fprintf('%d/%d\tti=%d mdi=%d\t%d %d\n',im,length(vli),ti,mdi,timeg(),int32(time(mdi)));%iig(im+moff*(nrowm*ncolm))
 end
if ti-toffm<=length(ind)  %% TODO check
% goes to new figure (if required)
  np = ntagm*(cell2mat(var{i}(6))-1)+ ns + nfig;
  if(im==1) vt{np-nfig}=[varshort0 tag]; end
%%  if cmpm==1
    gcf=figure(np); set(gcf, 'visible','on','Color','w'); hold on
...
%%    else      ix = 1+mod(im-1,ncolm); npc=floor((im-1)/ncolm); iy = ns;   end
...
%    ax_cmp=subplot(ntagm,nrowm,im);
  ...
  npc=moffc+npc+mofc*20;

  gcf=figure(npc); set(gcf,'Position',[0 0 130+ncolm*230 80+(nrowm+0)*180], 'visible',vis,'Color','w'); hold on
  figc=[figc npc];
    % geometry of sub-plot
  x0=0.06+(ix-1)*1.15*dxpm; y0=0.08+(iy-1)*1.01*dypm;
  ...
  if(str2num(VerMat)<8.4)
    ...
    gca=ax_cmp; plot_single_map; % repeat plotting
  end
