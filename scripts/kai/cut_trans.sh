...
# ---------------------
homed=$PWD
Nstart=2250 # initial time-step; skips trailer 
...
if [ $# -lt 1 ]; then
  ncpu=117     # sns configuration (#cpus)
else
  ncpu=$1
fi
...
#"Dissolved_Inorganic_Phosphorus_DIP_nutP_in_water"  "Phytplankton_Phosphorus_phyP_in_water" "Phytplankton_Nitrogen_phyN_in_water"   "mole_concentration_of_nitrate_in_soil" "Detritus_Carbon_detC_in_water" "Detritus_Nitrogen_detN_in_water" "Phytplankton_Carbon_phyC_in_water"  "Zooplankton_Carbon_zooC_in_water"
...
if [ $# -lt 2 ]; then
  outdir=~/sns/cut    # Where to put the extracted files
else
  outdir=$2
fi
...
tg='time,doy,getmGrid2D_y,getmGrid2D_x,getmGrid3D_y,getmGrid3D_x,getmGrid2D_Y,getmGrid2D_X,getmGrid3D_Y,getmGrid3D_X'
