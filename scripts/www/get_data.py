...
def get_url_bounds(dataset, corners):
    ...
    if (service == 'dods'):
        fullUrl = get_dods_url(dataset, corners)
    if (service == 'ncss'):
        fullUrl = get_ncss_url(dataset, corners)
    ...
    return string
    # print
    # 'https://rsg.pml.ac.uk/thredds/ncss/CCI_ALL-v1.0-DAILY?north=58&west=0&east=10&south=52&disableProjSubset=on&horizStride=1&time_start=1997-09-04T00%3A00%3A00Z&time_end=2012-07-31T00%3A00%3A00Z&timeStride=1&addLatLon=true'


def get_dods_url(dataset, corners):

    nc = netCDF4.Dataset(dataset['url'])
    if dataset.has_key('lon'):
        lonvar = dataset['lon']
    else:
        lonvar = 'lon'

    if dataset.has_key('lat'):
        lonvar = dataset['lat']
    else:
        latvar = 'lat'
