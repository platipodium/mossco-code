!> @file test_Regrid2d_GridToMesh2.F90
!! @brief explores Regrid capability on one from grid to ESMF mesh
!! @author Carsten Lemmen
!!

program test_Regrid2d_GridToMesh2
...
! Create UGrid
call ESMF_LogWrite('Creating ESMF mesh', ESMF_LOGMSG_INFO, rc=rc)
call ESMF_LogFlush(rc=rc)

mesh = ESMF_MeshCreate(filename='ne4np4-pentagons.nc',filetypeflag=ESMF_FILEFORMAT_SCRIP,rc=rc)
!mesh = ESMF_MeshCreate(filename='ne4np4-esmf.nc',filetypeflag=ESMF_FILEFORMAT_ESMFMESH,rc=rc)
if (rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT)
