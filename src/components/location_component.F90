!> @brief Implementation of an ESMF locstream input component
!>
...
#undef ESMF_FILENAME
#define ESMF_FILENAME "location_component.F90"

module location_component
...
#define ESMF_METHOD "InitializeP1"
  subroutine InitializeP1(gridComp, importState, exportState, parentClock, rc)
    ...
    if (isPresent) then
      ...
      ! Find the dimension that describes the number of locations
      hasDimension = .false.
      do i=1, itemCount
        if (nc%dimNames(i) == trim(dimensionName)) then
          hasDimension = .true.
          dimensionId = i
          exit
        endif
      ...
      ! try to take the first dimension that is not 'time'
      if (.not.hasDimension) then
        do i=1, itemCount
          if (nc%dimNames(i) /= 'time') then
            hasDimension = .true.
            dimensionName = trim(nc%dimNames(i))
            dimensionId = i
            exit
          endif
        enddo
      ...
      numLocationsOnThisPet = nc%dimLens(dimensionId)

      locstream=ESMF_LocStreamCreate(name=trim(name), localCount=numLocationsOnThisPet, rc=localrc)
      ...
      do i=1, itemCount
        if (trim(nc%variables(i)%standard_name) == 'latitude') then
          var => nc%getvarvar(trim(trim(nc%variables(i)%name)))
          call nc%getvar(field, var, itime=1, rc=localrc)
          exit
        endif
      enddo
