...
!> the call 'python ./create_coupling.py fabm_gotm.yaml'
!>
...
module toplevel_component
  ...
  type(ESMF_State), save    :: gotmExportState, gotmImportState
  type(ESMF_State), save    :: fabm_gotmExportState, fabm_gotmImportState
  ...
  subroutine Initialize(gridComp, importState, exportState, parentClock, rc)
    ...
    cplAlarmList(1)=ESMF_AlarmCreate(clock=clock,ringTime=startTime+alarmInterval, &
      ringInterval=alarmInterval, name='gotm--fabm_gotm--cplAlarm', rc=rc)
    if (rc /= ESMF_SUCCESS) call ESMF_Finalize(endflag=ESMF_END_ABORT, rc=rc)
    ...
    do i=1,numGridComp
      ...
      if (trim(name)=='gotmComp' .or. trim(name)=='fabm_gotmComp') then
